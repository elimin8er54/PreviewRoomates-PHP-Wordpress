...
function format_phone($number){
	if(isset($_SESSION['roommate_user_id'])) {
		...
		return "Log in to see phone";
	}
}



function get_datestatus($thedate) {
	$show = false;
	$status = "";
	if( time() - strtotime($thedate) < 24*60*60 ) {
		$status = "New Today";
		$show  = true;
	} else if( time() - strtotime($thedate) < 7*24*60*60 ) {
		$status = "New";
		$show = true;
	} 
	if($show == true) {
...
function get_savestatus($pdo, array $ar){
	session_start();

	$the_query = "";
	foreach ($ar as $id) {
		$the_query.=" OR roommate_adpref_adid=".$id;
	}
	
	$the_query=substr($the_query, 3);	
	
	
	$stmt = $pdo->prepare("SELECT roommate_adpref_issaved,roommate_adpref_adid
...
function get_savestatusprop($pdo, $ar){
	session_start();

	$the_query = "";
	foreach ($ar as $id) {
		$the_query.=" OR roommate_proppref_propertyid=".$id;
	}
	
	$the_query=substr($the_query, 3);	
	
	
	$stmt = $pdo->prepare("SELECT roommate_proppref_issaved,roommate_proppref_propertyid
	...
	foreach ($data as $row) {
		$vals[] = $row['roommate_proppref_propertyid'];
	}
