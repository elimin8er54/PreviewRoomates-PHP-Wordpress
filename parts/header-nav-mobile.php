...
<nav class="<?php echo implode(' ', $mobile_menu_classes ); ?>" id="nav-mobile" data-menu-id="<?php echo hu_get_menu_id( 'header'); ?>">
  ...
  <?php //endif; ?>
      ...
      <div class="nav-text"><?php apply_filters( 'hu_mobile_menu_text', '' );//put your mobile menu text here ?>
	  </div>
      <div class="nav-wrap container">
        <?php if ( hu_is_checked( 'mobile-search' ) ) : ?>
          <ul class="nav container-inner group mobile-search">
			...
			<div id="loginregister" style="float:left;"> 
				 <p style="color:white;float:left;"> <?php if(isset($_SESSION['roommate_user_id'])) { ?>
					Welcome, <?php echo $_SESSION['roommate_user_name'] . " "; ?></p> <a class="main_logout" href="#">Logout</a>
				  <?php } else { ?>
				  <a style = "float:left;" class = "button-action button-signin" href="<?php echo get_permalink( get_page_by_path( 'login' ) ); ?>">Login</a>
					<a style="float:left;margin-left:10px;color:white;" class = "button-" href="<?php echo get_permalink( get_page_by_path( 'register' ) ); ?>">Register</a> 
